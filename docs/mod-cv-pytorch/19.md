<title>Combining Computer Vision and NLP Techniques</title> <link rel="stylesheet" href="css/style.css" type="text/css">  Combining Computer Vision and NLP Techniques
...
![](assets/13719296-3e90-4669-951c-f09d3f972fac.png)
...
![](assets/dcf4219a-c3f3-4446-aac2-ee5fc4550d85.png)
...
![](assets/2a5d655d-c9e1-4912-827d-c2dd09139f39.png)
...
![](assets/b332f067-a9a2-4a1e-9e56-6ee1e0d73850.png)
...
![](assets/1b626eb8-7016-49a2-a658-82df4c61a1ba.png)
...
![](assets/84eff6d4-1b07-4f80-865d-1a68cb5c0803.png)

在上图中，您可以看到，虽然输入 **X** 和输出 **h** 与我们在*探索 RNN* 部分的结构中看到的相似，但是在 LSTM，输入和输出之间发生的计算是不同的。让我们来理解输入和输出之间发生的各种激活:

![](assets/1ea336bd-6c01-4a33-8976-44fc5eea0d92.png)
...
![](assets/a665003c-7845-404e-bd51-a10033a8f681.png)
...
![](assets/a4740087-7fc5-44fc-856a-1511898968f9.png)
...
![](assets/68f53be7-af1c-4716-a6a6-794099bda34e.png)

注意 *W* *[xi]* 和 *W* *[hi]* 分别代表与输入和前一个隐藏层相关联的权重。

修改门的激活计算如下:

![](assets/c7650351-7cc4-4a88-bb4c-f59b9432219d.png)
...
![](assets/6ed0fc6e-4715-4cc0-bc72-4e74c4c6557d.png)

最后，我们将激活的更新单元状态( *tanh(C [t] )* )乘以激活的输出值 *O [t]* ，以获得最终输出 *h [t]* ，在时间步长 *t* :

![](assets/6a0ebb8b-8061-41c5-bda3-44fa74879e9c.png)

![](assets/de7fe087-8d0c-44e6-b09f-237e2aa3fa99.png)
...
![](assets/543af9c0-42c3-425e-b12a-167768e6f9a2.png)
...
![](assets/767126a6-42c0-4022-a6ef-63068be66aa5.png)
...
![](assets/fd2e51ab-5a34-4cb8-bad1-bd2dcc42fbee.png)
...
![](assets/8d435638-7c32-45a4-991e-be25a7d32222.png)
...
![](assets/10f8c6af-3700-4101-a775-3a75acc9177f.png)
...
![](assets/41beb57d-7404-4eba-aae3-5ab7695c6d9c.png)
...
![](assets/c08258f5-8c39-4c23-b699-2ebf19a859f1.png)

*   打包的填充现在被传递给 LSTM，如下所示:

![](assets/113cc69a-aacc-4998-913c-ba71a7786141.png)
...
![](assets/6f477bac-8f8a-4920-8bf6-c2d523e2e9b8.png)
...
![](assets/88107d93-59e9-41f1-a26f-cf83290afe9e.png)
...
![](assets/b973570e-1832-4cdc-a2e4-53c495e4ec92.png)

然而，为了使计算简单，为了让我们理解 CTC 损失值是如何计算的，让我们假设图像只包含字符 *a* 而不包含单词 *ab* 的场景。此外，为了简化计算，我们假设只有**三个时间步长**:

![](assets/b0600076-4d20-4949-8a1d-a31e81657b5a.png)
...
![](assets/19c374a0-34a7-41f3-af8e-9046616620d9.png)
...
![](assets/fbe1c92a-5b4e-4de6-aedd-5d2c432e435b.png)
...
![](assets/14613998-d790-44dd-9dd2-bba379bd2ca3.png)
...
![](assets/175d1297-6c51-4068-b81f-6108da55aa23.png)

从图表中，我们可以看到，该模型在验证数据集上的单词准确率约为 80%。

此外，训练结束时的预测如下:

![](assets/d040ae8c-d7f4-47a9-b550-8e69f7cbe94d.png)
...
![](assets/cbc2174a-afa6-4418-9c20-f8bbfe5eff41.png)

因为每个向量的大小是 512，所以在它们之间进行矩阵乘法在计算上是昂贵的。因此，我们将这些向量中的每一个分成八个部分，对于键、查询和值张量中的每一个有八组(64×3)向量，其中 64 是从 512(嵌入大小)/ 8(多头)获得的，3 是序列长度:

![](assets/097d3838-af60-476c-96b9-8f64460a1e46.png)

注意会有八组![](assets/40292678-6884-4803-bef4-32a2c2e1d87f.png)、![](assets/aaf2cc5b-929a-4bc1-890d-25c3eeaa2350.png)等张量，因为有八个多头。

在每一部分中，我们首先执行键矩阵和查询矩阵之间的矩阵乘法。这样，我们最终得到一个 3 x 3 的矩阵。让它通过 softmax 激活。现在，我们有一个矩阵来显示每个单词相对于其他单词的重要性:

![](assets/09982b1b-aff5-436e-970d-7071efabdad2.png)

最后，我们执行前面的张量输出与值张量的矩阵乘法，以获得我们的自我注意操作的输出:

![](assets/5d7ab558-c827-4940-a56c-c3d62fef7d82.png)
...
![](assets/d7259db0-7efa-4a04-9da3-a93a94a80e65.png)
...
![](assets/1bf17fb4-ad3e-4698-a13f-326cdffb5e48.png)
...
![](assets/85d69870-c942-4689-b3f4-2aa25d7c8fb8.png)
...
![](assets/28350d8a-c5f7-4796-8a25-3da3956b1e14.png)
...
![](assets/cc5e3b46-69ad-4cfd-a909-a99c4d0a3ddb.png)
