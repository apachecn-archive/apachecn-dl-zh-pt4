<title>Advanced GANs to Manipulate Images</title> <link rel="stylesheet" href="css/style.css" type="text/css">  Advanced GANs to Manipulate Images
...
![](assets/257ffc98-7cae-44b3-9069-14af928929bb.png)
...
![](assets/30a0b7c4-0f1b-4735-b9be-95b1d32b32fa.png)
...
![](assets/98101a08-8428-4f68-9108-b68eff2698cd.png)

鉴别器架构概述如下:

![](assets/8e21885c-4b67-4d33-ad84-1f3c88290d6e.png)
...
![](assets/0aeb7a1d-2a25-4096-aa64-a01d11482680.png)
...
![](assets/bd05a779-cb67-45e7-aabe-f4967c597215.png)
...
![](assets/32731a8a-17ba-4cf5-b6a9-aa06cf79a3e9.png)
...
![](assets/287cc47b-00dc-4e7c-997a-d5fb7c8cc0b7.png)
...
![](assets/d56efaf2-918c-4b5e-85f7-68b73eda4734.png)
...
![](assets/092e7b73-1da6-460c-b310-2eefef030672.png)
...
![](assets/ca294c12-02d9-4827-bea7-01319815f867.png)

Python 脚本`encode_image.py`在较高层次上执行以下操作:

1.  在![](assets/c326fc77-2b14-42ad-810e-7113d2bf3c37.png)空间创建一个随机向量。
2.  用这个向量合成一个图像。
3.  使用 VGG 感知损失(与神经风格转换中使用的损失相同)将合成图像与原始输入图像进行比较。

4.  对![](assets/58be5dc4-1ff7-4a91-85bf-cd98badb2eab.png)随机向量进行反向传播，以减少固定迭代次数的损失。
5.  优化的![](assets/c38f8a2f-c953-457b-93f6-f6baaa9b4f86.png)矢量现在将合成一幅图像，VGG 为该图像给出与输入图像几乎相同的特征，因此合成的图像将看起来与输入图像相似。
...
![](assets/a3b5612f-ba27-41c2-beea-098174b594d0.png)

下面是用`idxs_to_swap`分别作为`slice(4,15)`和`slice (15,18)`的输出。

![](assets/77f9daa6-335a-4eec-97df-39f7e6c76fdf.png)

10.  接下来，我们推断一个样式向量，这样新的向量将只改变我们的自定义图像的微笑。为此，你需要计算移动![](assets/50e230e3-f0d8-41bf-93a4-2205e23bb9ec.png)潜在向量的正确方向。我们可以通过首先创建大量的假图像来实现这一点。然后使用 SVM 分类器来训练并找出图像中的人是否在微笑。因此，这个 SVM 创造了一个超平面，把微笑的脸和不微笑的脸分开。移动![](assets/35538611-b75e-4e92-95c9-587cfbcae790.png)所需的方向将垂直于该超平面，表示为`stylegan_ffhq_smile_w_boundary.npy`。实现细节可以在`InterfaceGAN/edit.py`代码本身中找到:
...
![](assets/d41c3baa-dad4-4b46-b03b-2f6681356fdc.png)
...
![](assets/69759245-8b60-4932-9c3f-0204e876cad0.png)
...
![](assets/7eaa234c-dc7b-4551-8c75-96fc2b77cfb2.png)
...
![](assets/b3da0dfe-4eb5-4cd1-b3c3-c362d169ad66.png)
