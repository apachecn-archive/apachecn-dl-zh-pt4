...
MLP 是前馈人工神经网络的典型例子。术语**前馈**指的是每一层都作为下一层的输入而没有循环，这与循环神经网络形成对比——这是一种架构，我们将在本章稍后讨论，并在*第 15 章*、*中更详细地讨论使用循环神经网络*对序列数据建模。术语*多层感知器*可能听起来有点混乱，因为这种网络架构中的人工神经元通常是 sigmoid 单元，而不是感知器。我们可以将 MLP 中的神经元视为逻辑回归单元，其返回值在 0 和 1 之间的连续范围内。
