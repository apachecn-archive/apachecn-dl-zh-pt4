...
现在，为了进行计算，我们可以简单地用张量对象作为函数参数来调用这个函数。注意 PyTorch 函数如`add`、`sub`(或`subtract`)、`mul`(或`multiply`)也允许我们以 py torch 张量对象的形式提供更高级别的输入。在下面的代码示例中，我们提供了标量输入(秩 0)以及秩 1 和秩 2 输入，如列表所示:
