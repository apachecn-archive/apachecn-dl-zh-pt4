...
*   在每一个小批量后修剪 critic 权重，使它们的绝对值小于常数 *c* 。这一要求加强了评论家所谓的 Lipschitz 约束，这使得使用 Wasserstein 距离成为可能(在论文中有更多关于这一点的内容)。在不深入细节的情况下，我们将只提到重量削减会导致不期望的行为。这些问题的一个成功的解决方案是梯度罚分(WGAN-GP，*瓦瑟斯坦GAN*，的改进训练)，它不会遇到同样的问题。
