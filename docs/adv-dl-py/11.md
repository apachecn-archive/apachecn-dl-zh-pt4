<title>Sequence-to-Sequence Models and Attention</title> <link rel="stylesheet" href="css/style.css" type="text/css"> 
...
![](assets/6ea7c879-71db-4d71-b835-5f96b75842cb.png)
...
![](assets/3be3cb9d-c46e-48d5-a46d-198ee13549ae.png)
...
*   它从一个热编码单词的输入序列开始，这些单词被转换成 *d [模型]* 维嵌入向量。嵌入向量进一步乘以![](assets/5ec0c397-cafb-43e7-92c8-0e5f2accc144.png)。
...
这个例子是基于[https://github.com/harvardnlp/annotated-transformer](https://github.com/harvardnlp/annotated-transformer)的。我们还要注意 PyTorch 1.2 引入了原生变压器模块(文档可在 https://pytorch.org/docs/master/nn.html#transformer-layers 的[获得)。不过，在这一节中，我们将从头开始实现转换器，以便更好地理解它。](https://pytorch.org/docs/master/nn.html#transformer-layers)
...
这相当简单:编码器由`EncoderBlock`的`self.blocks` : `N`个堆叠实例组成，其中每个实例作为下一个实例的输入。接下来是`LayerNorm`规范化`self.norm`(我们在*变压器模型*部分讨论了这些概念)。`forward`方法将数据张量`x`和一个实例`mask`作为输入，它阻塞了一些输入序列元素。正如我们在*变压器模型*部分所讨论的，屏蔽仅与模型的解码器部分相关，其中序列的未来元素尚不可用。在编码器中，掩码仅作为占位符存在。
...
为了获得一些视角，让我们用 *L* 表示变压器块的数量，用 *H* 表示隐藏的大小(之前用 *d [型号]* 表示)，用 *A* 表示自关注头的数量。论文作者实验了两种 BERT 构型:BERT[BASE](*L*= 12， *H* = 768， *A* = 12，总参数= 110M)和 BERT[LARGE](*L*= 24， *H* = 1024， *A* = 16，总参数= 340M
...
![](assets/78c7fc5c-275f-4ccc-975d-d483648c44ac.png)
...
<title>XLNet</title> <link rel="stylesheet" href="css/style.css" type="text/css"> 

# XLNet

作者指出，与单向自回归模型(如 transformer-XL)相比，具有去噪自动编码预训练的双向模型(如 BERT)可以实现更好的性能。但是正如我们在变压器部分的*双向编码器表示的*预训练*小节中提到的，`[MASK]`标记在预训练和微调步骤之间引入了差异。为了克服这些限制，transformer-XL 的作者提出了 XLNet(参见 *XLNet:在[https://arxiv.org/abs/1906.08237](https://arxiv.org/abs/1906.08237)的*用于语言理解的广义自回归预训练):一种广义的**自回归**预训练机制，通过最大化因子分解顺序的所有排列的预期可能性来实现双向上下文的学习。为了澄清，XLNet 建立在 transformer-XL 的 transformer 解码器模型上，并在自回归预训练步骤中引入了一种基于智能排列的双向上下文流机制。*

下图说明了模型如何处理具有不同因子分解顺序的相同输入序列。具体来说，它显示了一个具有两个堆叠模块和段级递归的变压器解码器( **mem** 字段):

![](assets/9bd20c60-51c4-4d3d-aebc-427ac09eed57.png)
...
为了形式化这个概念，让我们引入一些符号:![](assets/3a1b5515-8dcb-4562-a081-af1fa4ceadf2.png)是长度- *T* 索引序列[1，2，...，的所有可能排列的集合。。。，*T*；![](assets/8a903d12-8bac-423c-a8e6-3df5d4cee11e.png)是![](assets/3e93fec1-aaa9-44e0-9a1b-1011ba46b66f.png)的一种排列；![](assets/26099f4b-c07a-4e06-a6ba-6ab9a2bbe328.png)是该排列的第 *t* 个元素；![](assets/40c6c4da-6249-46ba-b758-3c5904d09767.png)是该排列的第一个 *t-1 个*元素，![](assets/2abdc5f8-5de3-429e-af1a-b29d5266370c.png)是给定当前排列![](assets/3b46504a-f316-4cf4-b448-dcd9ca95e065.png)(自回归任务，是模型的输出)的下一个单词![](assets/9b448615-aa0e-4e5e-9684-593bb672c733.png)的概率分布，其中θ是模型参数。然后，置换语言建模的目标如下:

![](assets/68a26555-aaca-4e73-bdad-7aaad53dd649.png)

它一次对输入序列的不同因式分解顺序进行一次采样，并试图最大化概率![](assets/715f3a1e-4047-41f6-9dd9-23af7a3786bb.png)—也就是说，增加模型预测正确单词的机会。参数θ在所有因式分解阶上共享；因此，模型将能够看到每一个可能的元素![](assets/585d38cf-1b5b-4953-872f-6e839aec3abc.png)，从而模拟双向上下文。同时这还是一个自回归函数，不需要`[MASK]`令牌。

我们还需要一块来充分利用基于排列的预训练。我们将从定义下一个词![](assets/5799d957-0303-4627-be58-7984f859a9c5.png)的概率分布开始，给定当前排列![](assets/0a1a99a8-7bb7-41f7-ae26-dc6b87be0247.png)(自回归任务，它是模型的输出)![](assets/1abef19f-67ad-4069-8b73-379d19a45b19.png)，它只是模型的 softmax 输出:

![](assets/ba32b664-9c7b-4447-837c-8d61e7de2889.png)

这里，![](assets/b9123cb8-acb5-4793-b2a8-5b424dadacb3.png)充当查询，![](assets/5e4e171d-48b8-4143-9a62-9938cd9309f9.png)是转换器在适当屏蔽后产生的隐藏表示，充当键值数据库。

接下来，让我们假设我们有两个因式分解顺序![](assets/a1d6ae4d-fc14-4d73-abb8-22182e3e58da.png)和![](assets/cee30934-7d1d-4415-97e9-76e18ed54cc8.png)，其中前两个元素相同，后两个交换。我们还假设 *t = 3—* 也就是说，模型必须预测序列的第三个元素。由于![](assets/177c0cfc-ffe1-497b-ad7c-39b542595b68.png)，我们可以看到![](assets/651f9b80-ff5e-498d-b921-9c3a06ce647d.png)在两种情况下都是一样的。因此，![](assets/9aac910e-9bcd-425f-ab54-708546ddc45c.png)。但这不是一个有效的结果，因为在第一种情况下，模型应该预测 *x [ 4 ]* ，而在第二种情况下， *x [ 1 ]* 。让我们记住，虽然我们在位置 3 预测了*x[1]和*x[4]，但它们仍然保持原来的位置编码。因此，我们可以修改当前公式，以包括预测元素的位置信息(对于 *x [1] 和*x[4]来说，位置信息会有所不同)，但排除实际的单词。换句话说，我们可以将模型的任务从*预测下一个单词*修改为*预测下一个单词，假设我们知道它的位置*。这样，两个样本因式分解阶数的公式将会不同。修改后的公式如下:****

![](assets/6dd9628b-fd10-48df-b4f6-8556ead937ac.png)

这里，![](assets/9996236e-51b9-4e90-8f7c-b15b9f2e8da6.png)是新的变换函数，它也包括位置信息![](assets/c4e80685-bb95-4c5f-9758-0bd6a64a85b6.png)。论文作者提出了一种称为双流自我注意的特殊机制来解决这一问题。顾名思义，它由两种结合的注意力机制组成:

*   内容表征 [![](assets/ceef0807-aa8b-47c3-b1bf-fd581358f7e6.png)] ，这是我们已经熟悉的注意机制。这种表示编码了上下文和内容[![](assets/c8cd3e08-f801-4f64-b2d5-cde6db23b1dd.png)本身。]
*   查询表示![](assets/e6387d63-4dbd-4c88-b999-b2ee6b4794d7.png)，它只能访问上下文信息![](assets/550a6616-0477-4b3a-99e6-30a58743f86a.png)和位置![](assets/bbe2dc8d-31f8-46e5-91e4-b026d1092c31.png)，而不能访问内容 [![](assets/ba4e9b81-5a07-44f2-8011-1451697da9ea.png)] ，正如我们之前提到的。

我鼓励你检查原始文件以获得更多的细节。在下一节中，我们将实现一个转换器语言模型的基本示例。

<title>Generating text with a transformer language model</title> <link rel="stylesheet" href="css/style.css" type="text/css"> 
...
3.  然后，我们将指定初始序列，将其标记化，并将其转换为模型兼容的输入`tokens_tensor`，其中包含一个标记列表:
