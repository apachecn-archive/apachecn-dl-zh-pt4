...
你会回忆起**函数近似** ( **FA** )使用从原始状态生成的一组特征来近似状态空间。**深度 Q 网络** ( **DQNs** )非常类似于带有神经网络的 FA，但是它们使用神经网络将状态直接映射到动作值，而不是使用一组生成的特征作为媒介。
...
这里， *s'* 是采取行动后的结果状态， *a* ，in 状态，*s*； *r* 是关联奖励；α是学习率；γ是贴现因子。此外， [![](img/781b37bb-fdcf-4f8f-a59e-7438e8d6cf73.png)] 意味着行为策略是贪婪的，其中选择状态*s’*中的最高 Q 值来生成学习数据。类似地，dqn 学习最小化以下误差项:

![](img/2e80a59e-1381-409a-a0fd-e477bf2e485f.png)

现在，目标变成了为每个可能的动作找到最佳网络模型以最佳近似状态值函数 *Q(s，a)* 。在这种情况下，我们试图最小化的损失函数类似于回归问题中的损失函数，即实际值和估计值之间的均方误差。
...
*   为新状态*s’*计算 *Q* 值`q_values_next`；然后，通过更新旧的 *Q* 值`q_values`，为动作![](img/0f9dd526-9c3d-4a3c-b2de-b0a019e3a40d.png)计算目标值。
...
深度 Q 学习使用更直接的模型(神经网络)来近似状态值，而不是使用一组中间人工特征。给定一个步骤，其中旧状态通过采取行动转移到新状态并获得奖励，训练 DQN 包括以下阶段:

*   使用神经网络模型来估计旧状态的 *Q* 值。
*   使用神经网络模型来估计新状态的 *Q* 值。
*   使用奖励和新的 *Q* 值更新行动的目标 Q 值，如 [![](img/e7041f0b-3588-40ff-a5db-dc73000512f9.png)]
...
使用一次一个样本的神经网络来近似 Q 值不是很稳定。你会记得，在 FA 中，我们加入了经验回放来提高稳定性。类似地，在这个菜谱中，我们将把经验回放应用到 DQNs。
...
这里， *s'* 是采取行动后的结果状态， *a* ，处于状态*s*； *r* 是关联奖励；α是学习率；γ是贴现因子。另外， [![](img/86da76aa-fe81-4d29-868c-a2dac845bea2.png)] 是目标网络的函数，Q 是预测网络的函数。
