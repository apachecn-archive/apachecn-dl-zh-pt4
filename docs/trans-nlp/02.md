...
在*第 1 章*、*开始使用变压器*的模型架构中，我们定义了原始变压器架构的构建模块。把最初的转换器想象成一个用乐高^(和 T5 积木搭建的模型。构造集包含诸如编码器、解码器、嵌入层、位置编码方法、多头关注层、掩蔽多头关注层、后层归一化、前馈子层和线性输出层等砖块。砖块有各种尺寸和形状。你可以花上几个小时用同一个组装工具组装各种模型！有些建筑只需要一些砖块。其他建筑会增加一个新的部分，就像我们用乐高组件为一个模型获得额外的砖块一样。)
...
我们将不再讨论第一章、*中描述的变压器的构建模块，开始使用变压器的模型架构*。你可以随时查阅*第 1 章*，复习转换器积木的某个方面。在这一节中，我们将关注 BERT 模型的具体方面。
...
训练一个转换器模型可能需要几个小时，甚至几天。设计架构和参数，并选择合适的数据集来训练变压器模型需要花费相当多的时间。
