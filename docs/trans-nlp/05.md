...
*Vaswani* 等人(2017)展示了变压器在 WMT 2014 年英德翻译任务和 WMT 2014 年英法翻译任务中的成果。转换器达到了最先进的 BLEU 评分。BLEU 将在本章的*用 BLEU* 评估机器翻译部分进行描述。
...
型号是带有编码器和解码器堆栈的变压器。每个堆栈包含`6`层和`8`头。`d_model=512`如在最初的转换器的架构中。
