...
当我们释放预训练的模型并观看它们执行下游**自然语言理解** ( **NLU** )任务时，转换器展示了它们的全部潜力。预训练和微调变压器模型需要花费大量的时间和精力，但当我们看到一个 3.55 亿参数的变压器模型在一系列 NLU 任务中发挥作用时，这种努力是值得的。
...
然后，我们探讨了如何测量多任务变压器的性能。在 NLP 的历史上，转换器为下游任务获得最高排名结果的能力是独一无二的。我们经历了艰难的强力胶任务，使转换器在胶水和强力胶排行榜上名列前茅。
