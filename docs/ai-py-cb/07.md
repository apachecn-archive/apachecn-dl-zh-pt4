...
在这个菜谱中，我们将在 PyTorch 中实现一个对抗性的自编码器。我们将实现监督和非监督方法，并显示结果。在无监督的方法中有一个很好的分类，在有监督的方法中，我们的编码器-解码器架构可以识别风格，这使我们能够进行风格转移。在这个食谱中，我们将使用 MNIST 计算机视觉公司的 *hello world* 数据集。
