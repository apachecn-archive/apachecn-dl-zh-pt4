<title>Patterns, Outliers, and Recommendations</title> <link rel="stylesheet" href="css/style.css" type="text/css">  Patterns, Outliers, and Recommendations
...
![](assets/cf1b680e-510c-471b-a063-c0036295dbde.png)
...
![](assets/02754259-7e11-4d76-8cfc-6a19784573c5.png)
...
![](assets/1f6dde32-3fb8-4b7c-9f1d-c7060cc6d7bf.png)
...
![](assets/7097097c-6dea-412c-a491-5a535dfc88a5.png)
...
![](assets/a2ad8f8e-a93b-4c82-b215-08c42d9df455.png)
...
![](assets/3651c14d-4b9b-4ac8-9293-a2b8101f54cc.png)
...
![](assets/7c45a89a-1996-4c4f-a869-4dc7369bf9a2.png)
...
![](assets/e49581a8-fb11-4b4a-902d-e2f23e88ac35.png)
...
![](assets/9af65a20-b46d-4b87-9a99-b3f01a2a5474.png)
...
![](assets/274696f4-58a3-40a2-952e-959b46161e5a.png)
...
更正式地说，我们可以将编码器定义为函数![](assets/cceac08e-9c97-44f0-bc14-82340bf370d3.png)，将解码器定义为函数![](assets/f2ab91f4-652d-49b7-880a-58a35110e5e8.png)。我们试图找到![](assets/4ff556c5-447c-4eeb-a8dd-ef0e76f6265a.png)和![](assets/339c4a30-f3ad-4d7e-a334-66ffee521b9f.png)，使得重建误差最小化；

![](assets/f012cf6c-7842-446d-9f3a-46c9acb63767.png)

自动编码器在中间网络层中表示数据，并且它们越接近基于中间表示的重构，它们就越不像是离群值。
...
![](assets/b428a0db-d2f2-437f-9d15-0b9fb09da16e.png)
...
![](assets/91661c56-aaae-4e42-8cc6-1c08744738cf.png)
...
![](assets/782734e6-0391-439c-bd33-4ba62c6f599a.png)
...
![](assets/b57e6718-d537-4cae-b6ce-138017220b1c.png)

我们得到以下建议:

![](assets/8257d8a3-4713-4b5e-b469-dc9b3aa82a38.png)
...
![](assets/53b50f3c-aad6-453e-95c9-3d9e07787b1f.png)
...
`spotlight`中的显式模型基于耶胡达·科伦等人提出的矩阵分解技术(在*推荐系统的矩阵分解技术*，2009)。基本思想是，用户-项目(交互)矩阵可以分解成两个分量，![](assets/30cd346a-e2c6-4fab-b244-3ae3d317cb46.png)和![](assets/d3edb87b-cb64-441d-8da4-7d503d3d42f0.png)，分别表示用户潜在因素和项目潜在因素，因此给定项目![](assets/b8cafe93-d461-4120-8df1-26c77ed61a27.png)和用户![](assets/6c7fa830-ca0a-42fe-8c15-35445af4e31c.png)的推荐可以计算如下:

![](assets/4a4d3d97-fbf1-454d-a708-f387edd25f54.png)
...
![](assets/5434beaa-96ea-4332-8ea8-fed1f65ab755.png)

在前面的函数中，我们有用户和项目的偏差项，而![](assets/748792ad-b5cf-4b86-99e4-a2d9a7885a5a.png)是 sigmoid 函数。

模型训练使数据的可能性最大化，条件是参数表达如下:

![](assets/715a6d4a-09ac-4fb0-9ebd-ecbc4ef2a389.png)
...
![](assets/8c6a7b40-e85a-4ab3-950c-a58c34b5ac75.png)
...
我们的数据集包含 284，807 个点。这意味着所有点之间的完整连接矩阵需要几百千兆字节(每个点 4 字节)，![](assets/c2ffa319-8c64-46e9-922a-6b272cff3262.png)。
...
我们在这个食谱中使用了鲁文算法。该算法于 2008 年由布朗德尔*等人*([https://arxiv.org/abs/0803.0476](https://arxiv.org/abs/0803.0476))发表。由于其时间复杂度是![](assets/8f50fad5-5b38-4055-b54e-b0494c083389.png)，卢万算法可以并且已经被用于大数据集，包括来自 Twitter 的包含 240 万个节点和 3800 万个链接的数据。

Louvain 算法的主要思想是通过连续地将社区合并在一起来以聚集的方式进行，从而增加它们的连通性。连通性通过边模块性![](assets/cd651623-1eea-4d47-a0c3-6510d7c7d524.png)来测量，边模块性是社区内连接到同一社区的其他顶点的边相对于其他社区的顶点的密度。顶点![](assets/aae57f62-676c-4db5-905a-7e4064c902e3.png)的社区的任何开关都有关联的![](assets/766320a0-c764-47a3-9ef4-0a65818930db.png)。在将每个顶点初始分配给它们自己的社区后，启发式算法分两步操作:贪婪地将顶点分配给社区，以及粗粒化。

*   对于所有顶点![](assets/60211e6a-f320-4dc8-9bb5-16b631454862.png)，将它们分配到社区，使![](assets/e7248b3a-8ff8-4182-bd06-6cccdf1cf6ca.png)尽可能地高。这个步骤可以重复几次，直到模块性没有改善。
...
整个算法的时间复杂度为![](assets/83121cdb-be4a-4a35-8e79-a30d97d7b1f0.png)，边 *m* ，顶点 *n* 。

### 信息熵

给定一个离散随机变量![](assets/de6d5b7f-f53f-4b22-87f4-dc2b89398466.png)，其可能值(或结果)![](assets/d7ba7d07-5891-4791-bafa-23b331a70c2b.png)以概率![](assets/bb4b527e-1e5e-4877-875b-03259d6a76e0.png)发生，![](assets/491e6796-8028-4b71-8efb-1f7ce3b9881a.png)的熵正式定义如下:

![](assets/4362bfae-04db-43bf-9c60-ed3134dcc2ce.png)
...
![](assets/7663ef5e-972f-430e-b909-efc2c2175b99.png)
