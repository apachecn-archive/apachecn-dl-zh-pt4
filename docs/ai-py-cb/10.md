<title>Natural Language Processing</title> <link rel="stylesheet" href="css/style.css" type="text/css">  Natural Language Processing
...
![](assets/e1ff8dab-e14b-4ecd-8223-865fec0b449a.png)
...
![](assets/9c86828c-52e3-4874-90d7-981bb90d8792.png)
...
![](assets/1efd0ca5-7f8b-44d1-a001-c2c8a6b5389a.png)
...
在*单词袋*部分，我们用`CountVectorizer`来统计单词。这给了我们一个向量![](assets/44e2f2df-1941-479d-ab2d-16eeb139b557.png)，其中![](assets/fa33ae63-50e8-44fb-9a40-4d3bc9a20d99.png)是词汇表中的单词数。在`transform()`能够基于词汇中记号(单词)的位置创建(稀疏)向量之前，必须在`CountVectorizer`的`fit()`阶段创建词汇。

通过对多个文档应用`CountVectorizer`，我们得到形状为![](assets/43576faf-0270-40f4-bccc-87fb8526b49d.png)的稀疏矩阵，其中![](assets/dcc5aab4-b1da-471e-bad9-e651b6474bb6.png)是语料库(文档的集合)，而![](assets/647ead55-3682-4f3d-928d-44af1d8bfcd0.png)是文档的数量。这个矩阵中的每个位置说明了某个标记在文档中出现的次数。在这个配方中，一个令牌对应一个单词，然而，它同样可以是一个字符或任何字符的集合。

一些单词可能出现在每个文档中；其他的可能只出现在一小部分文档中，表明它们更加具体和精确。这是 TFIDF 的直觉，如果语料库(文档集合)中的词频很低，计数(矩阵中的列)的重要性就会提高。

给定一组文档的术语![](assets/8562cff9-aa50-416a-9def-f79755765a60.png)的逆术语定义如下:

![](assets/04c10278-22df-4bc1-b90b-539e0648ba00.png)

这里的![](assets/2686c059-3f84-4c5f-b5f5-efe4a9bae38c.png)是一个文档![](assets/14778b5a-a60b-4c39-8904-05e8fb17293d.png)中一个术语![](assets/8fc9090d-e14e-4dda-954e-c91c0ba9792b.png)的计数，![](assets/9ae48337-e38e-4601-839a-c867bec58164.png)是![](assets/3b2b845a-66fa-4808-819c-5b008263395b.png)出现的文档数。您应该会看到 TFIDF 值随着![](assets/e2a4a99e-9466-4b5a-a8ac-78a50e58af85.png)减少。当一个术语出现在更多的文档中时，对数和 TFIDF 值接近 0。
...
首先，输入通过嵌入层和位置嵌入层，以便编码序列中记号的位置。标记嵌入按![](assets/35f98b55-821b-45c8-a75a-22519eddd6e9.png)(隐藏层大小的平方根)缩放，并添加到位置嵌入中。最后，退学申请正规化。
...
XLNet、OpenAI 的 GPT 和 GPT-2、Google 的 Reformer、OpenAI 的稀疏变压器和其他基于变压器的模型有一个共同点:它们是可生成的，因为建模选择——它们是自回归的，而不是自动编码的。这种自回归语言生成基于这样的假设，即给定长度为![](assets/3660a617-b11e-4ed3-8a5c-b76d66950499.png)的上下文序列，可以预测单词的概率，如下所示:

![](assets/0bfcdb76-4980-4b66-8a88-c9c0788b5dea.png)
