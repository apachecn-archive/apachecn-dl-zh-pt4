...
4.  调用函数来加载内容和样式图像。使用 dog 图像作为内容，使用 Matisse 图像作为样式，这两个图像都可以在 GitHub 存储库中找到:
    ...
    ```py
    content_img = image_loader("img/dog.jpg")
    style_img = image_loader("img/matisse.jpg")
    ```
...
如前所述，用于执行风格转换任务的架构是 19 层的 VGG 网络，也称为 VGG-19。预训练模型在模型的子包`torchvision`下可用。PyTorch 中保存的模型分为两部分，如下所述:
...
接下来，加载预训练的模型。如本章所述，VGG-19 是解决这类任务最常用的架构之一。它由 19 层组成，包括卷积层、池层和全连接层，其中，对于所讨论的任务，只使用了一些卷积层。考虑到 PyTorch 提供了一个包含几个预训练网络架构的子包，加载预训练模型的过程相当简单。
