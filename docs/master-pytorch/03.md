...
![Figure 1.1 – Deep learning model examples](Images/B12158_01_01.jpg)
...
![Figure 1.2 – Model performance versus dataset size](Images/B12158_01_02.jpg)
...
![Figure 1.3 – Fully connected layer](Images/B12158_01_03.jpg)
...
![Figure 1.4 – Convolutional layer](Images/B12158_01_04.jpg)
...
![Figure 1.5 – Recurrent layer](Images/B12158_01_05.jpg)
...
![Figure 1.6 – Deconvolutional layer](Images/B12158_01_06.jpg)
...
![Figure 1.7 – Pooling layer](Images/B12158_01_07.jpg)
...
![Figure 1.8 – Dropout layer](Images/B12158_01_08.jpg)
...
![Figure 1.9 – Different neural network architectures](Images/B12158_01_09.jpg)
...
![](Images/Formula_01_001.jpg)

该函数以图表形式显示如下:

![Figure 1.10 – Sigmoid function](Images/B12158_01_10.jpg)
...
![](Images/Formula_01_002.jpg)

该函数以图表形式显示如下:

![Figure 1.11 – TanH function](Images/B12158_01_11.jpg)

图 1.11–双曲正切函数

与 sigmoid 相反，在 TanH 激活功能的情况下，输出 *y* 从-1 变化到 1。因此，这种激活在情况下很有用，我们既需要正输出，也需要负输出。

*   **整流线性单元** ( **ReLUs** ): ReLUs 是比前两个更新的，简单地用表示如下:![](Images/Formula_01_003.jpg)

该函数以图表形式显示如下:

![Figure 1.12 – ReLU function](Images/B12158_01_12.jpg)

图 1.12–ReLU 功能

与 sigmoid 和 TanH 激活函数相比，ReLU 的一个显著特点是只要输入大于 0，输出就会随着输入不断增长。这防止了该函数的梯度像前面两个激活函数的情况一样减小到 0。尽管，每当输入为负时，输出和梯度都将为 0。

*   **泄漏 ReLU**:ReLU 通过输出 0 来完全抑制任何进入的负输入。然而，我们可能还想处理某些情况下的负面输入。泄漏 ReLUs 通过输出输入负输入的一小部分 *k* 来提供处理负输入的选项。这个分数 *k* 是这个激活函数的一个参数，数学上可以表示为:![](Images/Formula_01_004.jpg)

下图显示了泄漏 ReLU 的输入输出关系:

![Figure 1.13 – Leaky ReLU function](Images/B12158_01_13.jpg)
...
*   **Adadelta** :在 Adagrad 中，学习率的分母是一个由于在中每一个时间步长增加平方项而不断增值的项。这导致学习率衰减到几乎为零的小值。为了解决这个问题，Adadelta 引入了只计算前一时间步的梯度平方和的思想。其实我们可以把它表达为过去梯度的一个运行衰减平均值:![](Images/Formula_01_007.jpg)
...
![Figure 1.14 – PyTorch tensor storage](Images/B12158_01_14.jpg)
...
![Figure 1.15 – PyTorch tensor size](Images/B12158_01_15.jpg)
...
![Figure 1.16 – PyTorch tensor storage offset 1](Images/B12158_01_16.jpg)
...
![Figure 1.17 – PyTorch tensor storage offset 2](Images/B12158_01_17.jpg)
...
![Figure 1.18 – PyTorch tensor stride](Images/B12158_01_18.jpg)
...
2.  Next, we define the model architecture as shown in the following diagram:![Figure 1.19 – Neural network architecture](Images/B12158_01_19.jpg)
...
7.  And then we start the actual process of training the model for *k* number of epochs, and we also keep testing the model at the end of each training epoch:
    ...
    ![Figure 1.20 – Training logs](Images/B12158_01_20.jpg)
...
![Figure 1.21 – Sample handwritten image](Images/B12158_01_21.jpg)
...
![Figure 1.22 – PyTorch model prediction](Images/B12158_01_22.jpg)
