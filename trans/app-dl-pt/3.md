<title>C11865_03_ePub_Final_NT</title>
...
*   **输出层**:同样，任何人工神经网络只有一个输出层。它包含的单元数量取决于要开发的学习任务以及数据问题。如前所述，对于回归任务，只有一个单位，即预测值。另一方面，对于分类问题，考虑到模型的输出应该是属于每个类标签的一组特征的概率，单元的数量应该等于可用的类标签的数量。
