<title>Deep Feed-forward Neural Networks - Implementing Digit Classification</title>  
...
首先，让我们忽略 softmax，看看 softmax 函数内部有什么。`matmul`是矩阵相乘的张量流函数。如果你知道矩阵乘法([https://en.wikipedia.org/wiki/Matrix_multiplication](https://en.wikipedia.org/wiki/Matrix_multiplication))，你会明白这个计算是正确的，而且:
